...
@section('content')
    ...
    <!-- | -->
    <div class="pfm0 cachet0 edit0 for-active">
        <!-- | -->
        <!-- | -->
        <div class="edit">
            <!-- | -->
    ...
    <form action="{{ route('cachet.store') }}" data-dupl-val="true" data-edit-form="true" method="post" class="m-auto col-md-6 card p-2 rounded">
        @csrf
        <label class="form-group p-0 mb-4 InputLabel w-100">
            <input type="text" name="cachet" placeholder="Enter Cachet..."
                class="form-control AlterInput  propel-key-press-input-mendatory" autocomplete="off"
                value="{{ $modeldata['cachet'] }}">
            <span class="AlterInputLabel">Cachet</span>
        </label>
    ...
    <form action="{{ route('cachet.destroy', $modeldata['id']) }}" method="POST">
        @csrf
        @method('DELETE')
    ...
    <script>
        function cancelPage() {
            var url = "{{ route('cachet.index') }}";
            window.location.href = url;
        }
        var valRouteUrl = "{{ route('cachetValidation') }}";
        // function closePage(id){
        //   var url = "{{ route('cachet.edit', ':id') }}";
