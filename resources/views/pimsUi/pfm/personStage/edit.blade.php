...
@section('content')
    ...
    <!-- | -->
    <div class="pfm0 personStage0 edit0 for-active">
        <!-- | -->
        <!-- | -->
        <div class="edit">
            <!-- | -->
    ...
    <form action="{{ route('personStage.store') }}" data-dupl-val="true" data-edit-form="true" method="post"
        class="m-auto col-md-6 card p-2 rounded">
        @csrf
        <label class="form-group p-0 mb-4 InputLabel w-100">
            <input type="text" name="personStage" placeholder="Enter Person Stage..."
                class="form-control AlterInput  propel-key-press-input-mendatory" autocomplete="off"
                value="{{ $modeldata['personStage'] }}">
    ...
    <form action="{{ route('personStage.destroy', $modeldata['id']) }}" method="POST">
        @csrf
        @method('DELETE')
    ...
    <script>
        ...
        var valRouteUrl = "{{ route('personStageValidation') }}";
        // function closePage(id){
        //   var url = "{{ route('personStage.edit', ':id') }}";
