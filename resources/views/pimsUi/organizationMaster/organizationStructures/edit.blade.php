...
@section('content')
    ...
    <form action="{{ route('structure.store') }}" data-dupl-val="true" data-edit-form="true" method="post" class="m-auto col-md-6 card p-2 rounded">
        @csrf
        <label class="form-group p-0 mb-4 InputLabel w-100">
            <input type="text" name="structure" placeholder="Enter Structures..."
                class="form-control AlterInput  propel-key-press-input-mendatory" autocomplete="off"
                value="{{ $modeldata['structure'] }}">
        ...
        <input type="hidden" value="{{ $modeldata['structureId'] }}" name="id">
    ...
    <form action="{{ route('structure.destroy', $modeldata['structureId']) }}" method="POST">
        @csrf
        @method('DELETE')
    ...
    <script>
        ...
        var valRouteUrl = "{{ route('structureValidation') }}";
