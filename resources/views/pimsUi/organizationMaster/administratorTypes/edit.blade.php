...
@section('content')
    ...
    <!-- | -->
    <div class="organisation0 administratorTypes0 edit0 for-active">
        <!-- | -->
        <!-- | -->
        <div class="edit">
            <!-- | -->
    ...
    <form action="{{ route('administratorType.store') }}" data-dupl-val="true" data-edit-form="true" method="post"
        class="m-auto col-md-6 card p-2 rounded">
        @csrf
        <label class="form-group p-0 mb-4 InputLabel w-100">
            <input type="text" name="administratorType" placeholder="Administrator Type..."
                class="form-control AlterInput  propel-key-press-input-mendatory" autocomplete="off"
                value="{{ $modeldata['administratorType'] }}">
        ...
        <input type="hidden" value="{{ $modeldata['administratorTypeId'] }}" name="id">
    ...
    <form action="{{ route('administratorType.destroy', $modeldata['administratorTypeId']) }}" method="POST">
        @csrf
        @method('DELETE')
    ...
    <script>
        ...
        duplVal.on('input change', function() {
            ...
            $.ajax({
                url: "{{ route('administratorTypeValidation') }}",
                type: 'ajax',
                method: 'post',
                data: formData,
                contentType: false,
        ...
        //   var url = "{{ route('administratorType.edit', ':id') }}";
