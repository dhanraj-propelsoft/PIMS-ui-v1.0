...
@section('content')
    ...
    <form action="{{route('qualification.store')}}" method="post" class="m-auto col-md-6 card p-2 rounded">
        @csrf
        <label class="form-group p-0 mb-4 InputLabel w-100">
            <input type="text" name="qualification" required  placeholder="Enter Your Qualification..." class="form-control AlterInput "
                autocomplete="off" value="{{ $modeldata['qualification']}}">
    ...
    <form action="{{ route('qualification.destroy', $modeldata['id']) }}" method="POST">
        @csrf
        @method('DELETE')
