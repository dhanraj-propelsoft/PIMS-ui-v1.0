...
@section('content')
    ...
    <!-- | -->
    <div class="person-master0 gender0 edit0 for-active">
        <!-- | -->
        <!-- | -->
        <div class="edit">
            <!-- | -->
    ...
    <form action="{{ url('gender') }}" data-dupl-val='true' data-edit-form="true" method="post" class="m-auto col-md-6 card p-2 rounded">
        @csrf
        ...
        <input type="hidden" value="{{ $modeldata['genderId'] }}" name="id">
    ...
    <form action="{{ route('gender.destroy', $modeldata['genderId']) }}" method="POST">
        @csrf
        @method('DELETE')
    ...
    <script>
        ...
        var valRouteUrl = "{{ route('genderValidation') }}";
    </script>
@endsection
