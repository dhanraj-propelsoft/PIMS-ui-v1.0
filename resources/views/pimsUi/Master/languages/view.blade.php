...
@section('content')
    ...
    <!-- | -->
    <div class="common-master0 language0 view0 for-active"><!-- | -->
        <!-- | -->
        <div class="view"> <!-- | -->
            <!-- | --> <span>view</span> <!-- | -->
            <!-- | -->
    ...
    <div class="m-auto col-md-6 card p-2 rounded">
        <label class="form-group p-0 mb-4 InputLabel w-100">
            <input type="text" name="language" placeholder="Language..." class="form-control AlterInput " autocomplete="off"
                disabled value="{{ $modeldata['language'] }}">
        ...
        <div class="row justify-content-between  mx-1  mt-3">
            ...
            <button class="propelbtn propelbtncurved propelsubmit" onclick="viewPage(<?php echo $modeldata['languageId']; ?>)">Edit</button>
        </div>
    ...
    <script>
        function viewPage(id) {
            var url = "{{ route('language.edit', ':id') }}";
            url = url.replace(':id', id);
            window.location.href = url;
        }

        function cancelPage() {
            var url = "{{ route('language.index') }}";
            window.location.href = url;
        }
    </script>
@endsection
