...
{{-- This is For Navigation and Breadcrumbs --}}
 ...
 <!-- | -->   </div>                                                 <!-- | -->
   ...
   <form action="{{route('resourceEmailOtp')}}" class="col-sm-6 card m-auto p-5" method="POST"  id="resourceForm">
    {{csrf_field()}}
   <div >
    <p>
        This Mobile Number  <b>{{$person['mobile_no']}}</b>  and email  <b>{{$person['email']}} </b>has thes recordon our system By selecting one of them you ensure its your record.
    </p>
    <hr>
   
      <div class="RadioGroup">
        <input id="radio1" name="radio" class="identity" type="radio"/>
        <label for="radio1">
          <div class="customer-logo">
            <span> <p>{{Str::upper( mb_substr($person['first_name'], 0, 1))}}</p></span>
        </div>
        <div>
...
.RadioGroup input {
  ...
  visibility: hidden;
    ...
    <div class="col-12 d-flex justify-content-center mt-5">
        <a href="{{ route('createResource')}}" class="propelbtn propelcurved propelsubmit continue">Continue</a>
      </div>
   </div>
   <br>
