...
/* 03. propel Main Plugin */
$.propel = function (element, options) {
  var defaults = {};
  var plugin = this;
  plugin.settings = {};
  var $element = $(element);
  ...
  function init() {
    ...
    function setDirection() {
      try {
        if (localStorage.getItem("propel-direction")) {
          direction = localStorage.getItem("propel-direction");
        }
    ...
    if (typeof Chart !== "undefined") {
      Chart.defaults.global.defaultFontFamily = "'bitter', sans-serif";
      ...
      var centerTextPlugin = {
        ...
        beforeDraw: function (chart) {
          ...
          ctx.font = "36px" + " bitter, sans-serif";
          ...
          ctx.font = "14px" + " bitter, sans-serif";
          ctx.textBaseline = "middle";
    ...
    if ($().DataTable) {
      ...
      $(".data-table-feature").DataTable({
        ...
        drawCallback: function () {
          ...
          // $(".dataTables_wrapper .pagination").addClass("pagination-sm");
          // $('div.dataTables_wrapper div.dataTables_filter label').append("<i class='simple-icon-notebook propel-table-column-choose-icon'></i>");
          // $('div.dataTables_wrapper div.dataTables_filter label').css('position','relative');
          // $('div.dataTables_wrapper div.dataTables_filter label  .propel-table-column-choose-icon').css('position','absolute').css('right','0px').css('padding','10px');
        },
        language: {
          paginate: {
            previous: "<i class='simple-icon-arrow-left'></i>",
    ...
    if ($().smartWizard) {
      ...
      $("#smartWizardCustomButtons").on("leaveStep", function (e, anchorObject, currentStepIndex, nextStepIndex, stepDirection) {
        // if(anchorObject.prevObject.length - 1 == nextStepIndex){
        var WizardLength = Number($("#smartWizardCustomButtons .nav-item").length) - 2;

        if (nextStepIndex == "forward") {
          $('#prev-btn').show();
        }
        if (currentStepIndex >= WizardLength) {
          $('#ctn-btn').hide();
          $('#sub-btn').show();
          ...
          $('#ctn-btn').show();
          $('#sub-btn').hide();
          $('.wizard-move').removeAttr("type").attr("type", "button");
        }
      ...
      $("#smartWizardCustomButtons .prev-btn").on("click", function () {
        ...
        // var tab = $('#smartWizardCustomButtons .nav-item.active').text();
        // if (tab == "Organization") {
        //   $('#prev-btn').hide();
        // }
        return true;
      ...
      $("#smartWizardCustomButtons .next-btn").on("click", function () {
        ...
        if (typeof validCheck === 'function') {
          isValid = validCheck($(this)); // Call the function
...
$.fn.propel = function (options) {
  return this.each(function () {
    if (undefined == $(this).data("propel")) {
      var plugin = new $.propel(this, options);
      $(this).data("propel", plugin);
    }
  });
};
$("body").propel();
